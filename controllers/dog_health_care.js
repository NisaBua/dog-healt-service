...
exports.getFirstNode = (req, res, next) => {
    return (req, res, next) => {
        var query =
            'SELECT predict_disease_number,symptomNameTH,symptomNameEN,question_EN,question_TH,detail_TH,detail_EN  from predict_disease left join symptom on symptom.symptomNumber = predict_disease.current_symptom where first_node = true'
        try {
...
(exports.getNextNode = (req, res, next) => {
    return (req, res, next) => {
        var query =
            'SELECT predict_disease_number,symptomNameTH,symptomNameEN,predict_disease_result,question_EN,question_TH,detail_TH,detail_EN from predict_disease left join symptom on symptom.symptomNumber = predict_disease.current_symptom where previous_symptom = ? and previous_status = ?'
        try {
            database.query(
                query, [req.body.previous_symptom, req.body.previous_status],
                function(err, rows) {
                    if (err) {
                        ß
                        throw new Error(err)
...
(exports.updateSymptom = (req, res, next) => {
    return (req, res, next) => {
        ...
        var insertDisease =
            'UPDATE symptom SET question_TH = ? , question_EN =  ? , symptomNameEN = ? , symptomNameTH = ?  , detail_EN = ? , detail_TH = ?  WHERE symptomNumber = ?'
        try {
            database.query(
                insertDisease, [
